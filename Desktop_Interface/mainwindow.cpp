...
constexpr char kAboutString[]
    = "<h4>EspoTek Labrador</h4>"
#ifdef GIT_HASH_SHORT
      "Continuous Release<br>"
      "Git hash: <a href='https://github.com/espotek-org/Labrador/commits/" QUOTE(GIT_HASH_SHORT) "'>" QUOTE(
          GIT_HASH_SHORT) "</a><br>"
#endif
                          "Website: <a "
                          "href='http://espotek.com'>https://espotek.com</a><br>"
                          "Contact email: <a "
                          "href='mailto:admin@espotek.com'>admin@espotek.com</a>";
...
// Reset the device to ensure Labrador_libusbk gets handle!!
	...
	connect(ui->scaleHoriCheck, SIGNAL(clicked(bool)), this, SLOT(horiScaleEvent(bool)));
	connect(ui->scaleVertCheck, SIGNAL(clicked(bool)), this, SLOT(vertScaleEvent(bool)));
	// Screen Rotation.  Thanks, Hamlet.
	// https://forum.qt.io/topic/66240/how-to-detect-rotate-on-android
	screenPtr = QGuiApplication::primaryScreen();
	connect(screenPtr, SIGNAL(orientationChanged(Qt::ScreenOrientation)), this,
	    SLOT(screenRotateEvent(Qt::ScreenOrientation)));

	screenPtr->setOrientationUpdateMask(Qt::PortraitOrientation | Qt::LandscapeOrientation
	...
	// ui->controller_iso->driver->setBufferPtr(ui->bufferDisplay);
	connect(ui->debugButton1, SIGNAL(clicked()), ui->controller_iso->driver,
	    SLOT(avrDebug()));
	connect(ui->psuSlider, SIGNAL(voltageChanged(double)), ui->controller_iso->driver,
	    SLOT(setPsu(double)));
	connect(ui->controller_iso, SIGNAL(setGain(double)), ui->controller_iso->driver,
	    SLOT(setGain(double)));
	connect(ui->controller_fg, &functionGenControl::functionGenToUpdate,
	    ui->controller_iso->driver, &genericUsbDriver::setFunctionGen);
	connect(ui->bufferDisplay, SIGNAL(modeChange(int)), ui->controller_iso->driver,
	    SLOT(setDeviceMode(int)));
	connect(ui->bufferDisplay, &bufferControl::modeChange, this,
	    [this]()
	    {
		    // Force a trigger refresh
		    ui->controller_iso->setTriggerLevel(ui->triggerLevelValue->value());
	    });
	connect(ui->bufferDisplay, SIGNAL(updateDig(int)), ui->controller_iso->driver,
	    SLOT(newDig(int)));

	// Set the settings again!
	connect(ui->controller_iso->driver, SIGNAL(gainBuffers(double)), ui->controller_iso,
	    SLOT(gainBuffers(double)));
	connect(ui->controller_iso->driver, SIGNAL(disableWindow(bool)), this,
	    SLOT(setEnabled(bool)));
	connect(ui->controller_iso->driver, SIGNAL(sendClearBuffer(bool, bool, bool)),
	    ui->controller_iso, SLOT(clearBuffers(bool, bool, bool)));
	// connect(ui->controller_iso->driver, SIGNAL(startIsoTimer()), ui->controller_iso,
	// SLOT(startTimer()));
	connect(ui->controller_iso->driver, SIGNAL(setVisible_CH2(bool)), ui->controller_iso,
	    SLOT(setVisible_CH2(bool)));
	// connect(ui->controller_iso->driver, SIGNAL(enableMMTimer()), ui->controller_iso,
	// SLOT(enableMM()));
	connect(ui->controller_iso->driver, SIGNAL(checkXY(bool)), ui->xyDisplayLabel,
	    SLOT(setChecked(bool)));
	connect(ui->controller_iso->driver, SIGNAL(disableWindow(bool)), ui->deviceConnected,
	    SLOT(connectedStatusChanged(bool)));
	connect(ui->controller_iso->driver, SIGNAL(upTick()), ui->controller_iso,
	    SLOT(timerTick()));
	connect(ui->controller_iso->driver, SIGNAL(connectedStatus(bool)),
	    ui->deviceConnected, SLOT(connectedStatusChanged(bool)));
	connect(ui->controller_iso->driver, SIGNAL(signalFirmwareFlash(void)),
	    ui->deviceConnected, SLOT(flashingFirmware(void)));
	...
	connect(ui->controller_iso, SIGNAL(multimeterREnabled(int)), this,
	    SLOT(rSourceIndexChanged(int)));
	connect(ui->controller_iso, SIGNAL(multimeterRMS(double)), ui->multimeterRmsDisplay,
	    SLOT(display(double)));
	connect(ui->controller_iso, SIGNAL(sendMultimeterLabel4(QString)),
	    ui->multimeterRmsLabel, SLOT(setText(QString)));
	connect(ui->controller_iso, SIGNAL(sendVRMS_CH1(double)),
	    ui->voltageInfoRmsDisplay_CH1, SLOT(display(double)));
	connect(ui->controller_iso, SIGNAL(sendVRMS_CH2(double)),
	    ui->voltageInfoRmsDisplay_CH2, SLOT(display(double)));

	connect(ui->controller_iso, SIGNAL(mainWindowPleaseDisableSerial(int)), this,
	    SLOT(serialEmergencyDisable(int)));

	connect(ui->controller_iso->driver, SIGNAL(signalFirmwareFlash(void)),
	    ui->deviceConnected, SLOT(flashingFirmware(void)));
	connect(ui->controller_iso->internalBuffer375_CH1, SIGNAL(fileIOinternalDisable()),
	    this, SLOT(fileLimitReached_CH1()));
	connect(ui->controller_iso->internalBuffer750, SIGNAL(fileIOinternalDisable()), this,
	    SLOT(fileLimitReached_CH1()));
	connect(ui->controller_iso->internalBuffer375_CH2, SIGNAL(fileIOinternalDisable()),
	    this, SLOT(fileLimitReached_CH2()));

	connect(ui->controller_iso, SIGNAL(showRealtimeButton(bool)), ui->realTimeButton,
	    SLOT(setVisible(bool)));
	connect(ui->realTimeButton, SIGNAL(pressed()), ui->controller_iso,
	    SLOT(disableFileMode()));
	...
	connect(ui->actionHexDisplay, &QAction::toggled, ui->controller_iso,
	    &isoDriver::setHexDisplay_CH1);
	connect(ui->actionHexDisplay_2, &QAction::toggled, ui->controller_iso,
	    &isoDriver::setHexDisplay_CH2);
...
#ifndef PLATFORM_ANDROID
	connect(ui->offsetSpinBox_CH1, SIGNAL(valueChanged(double)), ui->controller_iso,
	    SLOT(offsetChanged_CH1(double)));
	connect(ui->offsetSpinBox_CH2, SIGNAL(valueChanged(double)), ui->controller_iso,
	    SLOT(offsetChanged_CH2(double)));

	connect(ui->attenuationComboBox_CH1, SIGNAL(currentIndexChanged(int)),
	    ui->controller_iso, SLOT(attenuationChanged_CH1(int)));
	connect(ui->attenuationComboBox_CH2, SIGNAL(currentIndexChanged(int)),
	    ui->controller_iso, SLOT(attenuationChanged_CH2(int)));
#endif
	connect(ui->controller_iso, &isoDriver::enableCursorGroup, this,
	    &MainWindow::cursorGroupEnabled);
	...
	connect(spectrumMinXSpinbox, QOverload<double>::of(&espoSpinBox::valueChanged),
	    ui->controller_iso, &isoDriver::setMinSpectrum);
	connect(spectrumMaxXSpinbox, QOverload<double>::of(&espoSpinBox::valueChanged),
	    ui->controller_iso, &isoDriver::setMaxSpectrum);
	connect(windowingComboBox, QOverload<int>::of(&QComboBox::currentIndexChanged),
	    ui->controller_iso, &isoDriver::setWindowingType);

	connect(spectrumMinXSpinbox, QOverload<double>::of(&espoSpinBox::valueChanged),
	    spectrumMaxXSpinbox, &espoSpinBox::setMinimum);
	connect(spectrumMaxXSpinbox, QOverload<double>::of(&espoSpinBox::valueChanged),
	    spectrumMinXSpinbox, &espoSpinBox::setMaximum);

	connect(spectrumMinXSpinbox, SIGNAL(valueChanged(double)), spectrumMinXSpinbox,
	    SLOT(changeStepping(double)));
	connect(spectrumMaxXSpinbox, SIGNAL(valueChanged(double)), spectrumMaxXSpinbox,
	    SLOT(changeStepping(double)));
	...
	connect(freqRespMinXSpinbox, QOverload<double>::of(&espoSpinBox::valueChanged),
	    ui->controller_iso, &isoDriver::setMinFreqResp);
	connect(freqRespMaxXSpinbox, QOverload<double>::of(&espoSpinBox::valueChanged),
	    ui->controller_iso, &isoDriver::setMaxFreqResp);
	connect(freqRespStepSpinbox, QOverload<double>::of(&espoSpinBox::valueChanged),
	    ui->controller_iso, &isoDriver::setFreqRespStep);
	connect(freqRespTypeComboBox, QOverload<int>::of(&QComboBox::currentIndexChanged),
	    ui->controller_iso, &isoDriver::setFreqRespType);
	connect(freqRespRestartButton, &QPushButton::clicked, ui->controller_iso,
	    &isoDriver::restartFreqResp);

	connect(freqRespMinXSpinbox, QOverload<double>::of(&espoSpinBox::valueChanged),
	    freqRespMaxXSpinbox, &espoSpinBox::setMinimum);
	connect(freqRespMaxXSpinbox, QOverload<double>::of(&espoSpinBox::valueChanged),
	    freqRespMinXSpinbox, &espoSpinBox::setMaximum);

	connect(freqRespMinXSpinbox, SIGNAL(valueChanged(double)), freqRespMinXSpinbox,
	    SLOT(changeStepping(double)));
	connect(freqRespMaxXSpinbox, SIGNAL(valueChanged(double)), freqRespMaxXSpinbox,
	    SLOT(changeStepping(double)));
	connect(freqRespStepSpinbox, SIGNAL(valueChanged(double)), freqRespStepSpinbox,
	    SLOT(changeStepping(double)));
...
{
	// ui->scopeAxes->yAxis->setAutoTickCount((ui->scopeAxes->height() + TICK_SEPARATION /
	// 2) / TICK_SEPARATION); ui->scopeAxes->xAxis->setAutoTickCount((ui->scopeAxes->width()
	// + TICK_SEPARATION / 2) / TICK_SEPARATION);
	...
	if (forceSquare)
	{
		...
		{
			ui->scopeAxes->move(
			    ui->scopeAxes->x(), ui->scopeAxes->y() + (tempHeight - tempWidth) / 2);
		}
	}
...
void MainWindow::menuSetup()
{
	...
	connect(ui->actionAutoV, SIGNAL(toggled(bool)), ui->controller_iso,
	    SLOT(setAutoMultimeterV(bool)));
	connect(ui->actionV, SIGNAL(toggled(bool)), ui->controller_iso,
	    SLOT(setForceVolts(bool)));
	connect(ui->actionMV, SIGNAL(toggled(bool)), ui->controller_iso,
	    SLOT(setForceMillivolts(bool)));
	connect(ui->actionAutoI, SIGNAL(toggled(bool)), ui->controller_iso,
	    SLOT(setAutoMultimeterI(bool)));
	connect(ui->actionA, SIGNAL(toggled(bool)), ui->controller_iso, SLOT(setForceAmps(bool)));
	connect(ui->actionMA, SIGNAL(toggled(bool)), ui->controller_iso,
	    SLOT(setForceMilliamps(bool)));
	connect(ui->actionAutoR, SIGNAL(toggled(bool)), ui->controller_iso,
	    SLOT(setAutoMultimeterR(bool)));
	connect(ui->actionOhm, SIGNAL(toggled(bool)), ui->controller_iso,
	    SLOT(setForceOhms(bool)));
	connect(ui->actionKOhm, SIGNAL(toggled(bool)), ui->controller_iso,
	    SLOT(setForceKiloOhms(bool)));
	connect(ui->actionAutoC, SIGNAL(toggled(bool)), ui->controller_iso,
	    SLOT(setAutoMultimeterC(bool)));
	connect(ui->actionNF, SIGNAL(toggled(bool)), ui->controller_iso,
	    SLOT(setForceNFarads(bool)));
	connect(ui->action_F, SIGNAL(toggled(bool)), ui->controller_iso,
	    SLOT(setForceUFarads(bool)));
	...
	uartBaudGroup_CH1->addAction(ui->action31250);
	...
	uartBaudGroup_CH2->addAction(ui->action31250_2);
...
{
	connect(ui->actionMax, SIGNAL(toggled(bool)), ui->voltageInfoMaxLabel_CH1,
	    SLOT(setVisible(bool)));
	connect(ui->actionMax, SIGNAL(toggled(bool)), ui->voltageInfoMaxDisplay_CH1,
	    SLOT(setVisible(bool)));

	connect(ui->actionMin, SIGNAL(toggled(bool)), ui->voltageInfoMinLabel_CH1,
	    SLOT(setVisible(bool)));
	connect(ui->actionMin, SIGNAL(toggled(bool)), ui->voltageInfoMinDisplay_CH1,
	    SLOT(setVisible(bool)));

	connect(ui->actionMean, SIGNAL(toggled(bool)), ui->voltageInfoMeanLabel_CH1,
	    SLOT(setVisible(bool)));
	connect(ui->actionMean, SIGNAL(toggled(bool)), ui->voltageInfoMeanDisplay_CH1,
	    SLOT(setVisible(bool)));

	connect(ui->actionRMS, SIGNAL(toggled(bool)), ui->voltageInfoRmsLabel_CH1,
	    SLOT(setVisible(bool)));
	connect(ui->actionRMS, SIGNAL(toggled(bool)), ui->voltageInfoRmsDisplay_CH1,
	    SLOT(setVisible(bool)));
	...
	connect(ui->actionMax_2, SIGNAL(toggled(bool)), ui->voltageInfoMaxLabel_CH2,
	    SLOT(setVisible(bool)));
	connect(ui->actionMax_2, SIGNAL(toggled(bool)), ui->voltageInfoMaxDisplay_CH2,
	    SLOT(setVisible(bool)));

	connect(ui->actionMin_2, SIGNAL(toggled(bool)), ui->voltageInfoMinLabel_CH2,
	    SLOT(setVisible(bool)));
	connect(ui->actionMin_2, SIGNAL(toggled(bool)), ui->voltageInfoMinDisplay_CH2,
	    SLOT(setVisible(bool)));

	connect(ui->actionMean_2, SIGNAL(toggled(bool)), ui->voltageInfoMeanLabel_CH2,
	    SLOT(setVisible(bool)));
	connect(ui->actionMean_2, SIGNAL(toggled(bool)), ui->voltageInfoMeanDisplay_CH2,
	    SLOT(setVisible(bool)));

	connect(ui->actionRMS_2, SIGNAL(toggled(bool)), ui->voltageInfoRmsLabel_CH2,
	    SLOT(setVisible(bool)));
	connect(ui->actionRMS_2, SIGNAL(toggled(bool)), ui->voltageInfoRmsDisplay_CH2,
	    SLOT(setVisible(bool)));
...
void MainWindow::on_action31250_toggled(bool arg1)
{
	if (arg1)
	{
		ui->controller_iso->baudRate_CH1 = 31250;
	}
}
...
void MainWindow::on_action31250_2_toggled(bool arg1)
{
	if (arg1)
	{
		ui->controller_iso->baudRate_CH2 = 31250;
	}
}
...
void MainWindow::cycleBaudRate_CH1()
{
	...
	if (currentAction == ui->action31250)
	{
		ui->action31250->setChecked(true);
		return;
	}
...
void MainWindow::cycleBaudRate_CH2()
{
	...
	if (currentAction == ui->action31250_2)
	{
		ui->action31250_2->setChecked(true);
		return;
	}
...
void MainWindow::initShortcuts()
{
	...
	shortcut_cycleBaudRateBackwards_CH1
	    = new QShortcut(QKeySequence("Ctrl+Shift+B"), ui->menuBar);
	shortcut_cycleBaudRate_CH2 = new QShortcut(QKeySequence("Ctrl+Alt+B"), ui->menuBar);
	shortcut_cycleBaudRateBackwards_CH2
	    = new QShortcut(QKeySequence("Ctrl+Shift+Alt+B"), ui->menuBar);
	...
	connect(shortcut_cycleBaudRateBackwards_CH1, SIGNAL(activated()), this,
	    SLOT(cycleBaudRateBackwards_CH1()));
	connect(shortcut_cycleBaudRate_CH2, SIGNAL(activated()), this, SLOT(cycleBaudRate_CH2()));
	connect(shortcut_cycleBaudRateBackwards_CH2, SIGNAL(activated()), this,
	    SLOT(cycleBaudRateBackwards_CH2()));
	connect(shortcut_snapshot_CH1, SIGNAL(activated()), this,
	    SLOT(on_actionSnapshot_CH1_triggered()));
	connect(shortcut_snapshot_CH2, SIGNAL(activated()), this,
	    SLOT(on_actionSnapshot_CH2_triggered()));
	...
	connect(shortcut_CtrlArrowDown, SIGNAL(activated()), this,
	    SLOT(ctrlArrowDownTriggered()));
	...
	connect(shortcut_CtrlArrowRight, SIGNAL(activated()), this,
	    SLOT(cycleDelayRight_large()));

	connect(shortcut_snapScopeToCursors, SIGNAL(activated()), this,
	    SLOT(on_actionSnap_to_Cursors_triggered()));
	connect(shortcut_manualRange, SIGNAL(activated()), this,
	    SLOT(on_actionEnter_Manually_triggered()));
...
void MainWindow::cycleDelayRight()
{
	...
	if (ui->controller_iso->display->delay < 0)
...
{
	qDebug() << "LEFT";
	double mws = ui->controller_iso->fileModeEnabled ? ui->controller_iso->daq_maxWindowSize
...
{
	ui->controller_iso->display->delay = 0;
	scopeRangeEnterDialog dialog(this, ui->controller_iso->display->topRange,
	    ui->controller_iso->display->botRange, ui->controller_iso->display->window,
	    ui->controller_iso->display->delay);
	dialog.setModal(true);
	connect(&dialog, SIGNAL(yTopUpdated(double)), ui->controller_iso,
	    SLOT(setTopRange(double)));
	connect(&dialog, SIGNAL(yBotUpdated(double)), ui->controller_iso,
	    SLOT(setBotRange(double)));
	connect(&dialog, SIGNAL(windowUpdated(double)), ui->controller_iso,
	    SLOT(setTimeWindow(double)));
	connect(&dialog, SIGNAL(delayUpdated(double)), ui->controller_iso,
	    SLOT(setDelay(double)));
	dialog.exec();
}
...
void MainWindow::readSettingsFile()
{
	...
	double calibrate_gain_ch1
	    = settings->value("CalibrateGainCH1", R4 / (R3 + R4)).toDouble();
	double calibrate_gain_ch2
	    = settings->value("CalibrateGainCH2", R4 / (R3 + R4)).toDouble();
	...
	if (!dt_AlreadyAskedAboutCalibration
	    && ((calibrate_vref_ch1 == 1.65) || (calibrate_vref_ch2 == 1.65)
	        || (calibrate_gain_ch1 == R4 / (R3 + R4))
	        || (calibrate_gain_ch2 == R4 / (R3 + R4))))
	{
		...
		calibrationMessages->setText("No calibration data detected!\nWould you like to "
		                             "run calibration once connected?");
...
void MainWindow::reinitUsb(void)
{
	...
	ui->controller_iso->driver->saveState(&reinitdeviceMode, &reinitScopeGain,
	    &reinitCurrentPsuVoltage, &reinitDigitalPinState);
...
void MainWindow::reinitUsbStage2(void)
{
	...
	connect(ui->debugButton1, SIGNAL(clicked()), ui->controller_iso->driver,
	    SLOT(avrDebug()));
	connect(ui->debugButton3, SIGNAL(clicked()), ui->controller_iso->driver,
	    SLOT(bootloaderJump()));
	connect(ui->psuSlider, SIGNAL(voltageChanged(double)), ui->controller_iso->driver,
	    SLOT(setPsu(double)));
	connect(ui->controller_iso, SIGNAL(setGain(double)), ui->controller_iso->driver,
	...
	connect(ui->controller_iso->driver, SIGNAL(signalFirmwareFlash(void)),
	    ui->deviceConnected, SLOT(flashingFirmware(void)));
	connect(ui->controller_iso->driver, SIGNAL(initialConnectComplete()), this,
	    SLOT(resetUsbState()));
	...
	connect(ui->controller_iso->driver, SIGNAL(calibrateMe()), this,
	    SLOT(on_actionCalibrate_triggered()));
...
void MainWindow::resetUsbState(void)
{
	...
	ui->controller_iso->driver->setFunctionGen(
	    ChannelID::CH1, ui->controller_fg->getChannelController(ChannelID::CH1));
	ui->controller_iso->driver->setFunctionGen(
	    ChannelID::CH2, ui->controller_fg->getChannelController(ChannelID::CH2));
...
#ifdef PLATFORM_ANDROID
// Should be called "High Resolution mode".  This function has been comandeered for
// Android devices with 1080p or higher resolutions.
...
bool MainWindow::gestureFilter(QGestureEvent* event)
{
	...
	if (capturedGesture->gestureType() == Qt::PinchGesture)
	{
		...
		{
			wheelEmu = new QWheelEvent(
			    point, (embiggen ? 120 : -120), 0, Qt::ControlModifier, Qt::Vertical);
		}
		else
		{
...
void MainWindow::on_actionCalibrate_triggered()
{
	...
	if (!ui->controller_iso->driver->connected)
	{
		...
		calibrationMessages->setText(
		    "You need to connect the board before calibrating it!");
	...
	if (ui->controller_iso->driver->deviceMode != 4)
	{
		...
		calibrationMessages->setText(
		    "The calibration sequence requires all devices to be turned off, except for "
		    "the oscilloscope CH1 and CH2.  Is it OK for me to change your workspace?");
	...
	calibrationMessages->setText("Please disconnect all wires from your Labrador board "
...
void MainWindow::calibrateStage2()
{
	...
	{
		calibrationMessages->setText(
		    "Calibration has been abandoned due to out-of-range values.  Both channels "
		    "should show approximately 1.6V.  Please disconnect all wires from your "
		    "Labrador board and try again.");
		calibrationMessages->exec();
		return;
	}
	...
	calibrationMessages->setText(
	    "Please connect both oscilloscope channels to the outer shield of the USB "
	    "connector then press OK to continue.");
	calibrationMessages->exec();
...
void MainWindow::calibrateStage3()
{
	...
	if ((vMeasured_CH1 > 0.3) | (vMeasured_CH1 < -0.3) | (vMeasured_CH2 > 0.3)
	    | (vMeasured_CH2 < -0.3))
	{
		calibrationMessages->setText(
		    "Calibration has been abandoned due to out-of-range values.  Both channels "
		    "should show approximately 0V.  Please try again.");
		calibrationMessages->exec();
		return;
	}
	...
	ui->controller_iso->frontendGain_CH1
	    = (vref_CH1 - vMeasured_CH1) * (ui->controller_iso->frontendGain_CH1) / vref_CH1;
	ui->controller_iso->frontendGain_CH2
	    = (vref_CH2 - vMeasured_CH2) * (ui->controller_iso->frontendGain_CH2) / vref_CH2;
	qDebug() << "New gain (CH1) = " << ui->controller_iso->frontendGain_CH1;

	ui->controller_iso->internalBuffer375_CH1->m_frontendGain
	    = (vref_CH1 - vMeasured_CH1) * (ui->controller_iso->frontendGain_CH1) / vref_CH1;
	ui->controller_iso->internalBuffer750->m_frontendGain
	    = (vref_CH1 - vMeasured_CH1) * (ui->controller_iso->frontendGain_CH1) / vref_CH1;
	ui->controller_iso->internalBuffer375_CH2->m_frontendGain
	    = (vref_CH2 - vMeasured_CH2) * (ui->controller_iso->frontendGain_CH2) / vref_CH2;
...
void MainWindow::on_actionExportImage_triggered()
{
	...
	dialog.setNameFilter(
	    "PDF files (*.pdf);;JPEG files (*.jpg);;PNG files (*.png);;BMP files (*.bmp)");
...
void MainWindow::on_actionRecord_CH1_triggered(bool checked)
{
	...
	if (ui->controller_iso->driver->deviceMode != 6)
	{
		...
		ui->controller_iso->internalBuffer375_CH1->enableFileIO(
		    output375_CH1, daq_num_to_average, daq_max_file_size);
	}
	else
	{
		output750 = new QFile(fileName);
		ui->controller_iso->internalBuffer750->enableFileIO(
		    output750, daq_num_to_average, daq_max_file_size);
	}
...
void MainWindow::on_actionRecord_CH2_triggered(bool checked)
{
	...
	ui->controller_iso->internalBuffer375_CH2->enableFileIO(
	    output375_CH2, daq_num_to_average, daq_max_file_size);

	if ((checked) && (ui->controller_iso->driver->deviceMode != 4))
		QMessageBox::warning(nullptr,
		    "Warning", "CH2 is disabled.  The DAQ will NOT RECORD any data from from CH2 until it has been enabled!");
}
...
void MainWindow::on_actionDAQ_Settings_triggered()
{
	...
	connect(&df, SIGNAL(updatedMaxFileSize(qulonglong)), this,
	    SLOT(daq_updatedMaxFileSize(qulonglong)));
...
void MainWindow::fileLimitReached_CH1(void)
{
	...
	QString recordingStoppedMessage
	    = QString("Maximum file size limit of %1MB reached.  Data Acquisition Stopped.")
	          .arg(daq_max_file_size / 1000000);
...
void MainWindow::dpiAutoScaling()
{
	...
	if (numPixels > (1280 * 720 * 1.5))
	{
		...
		on_actionOld_Person_Mode_triggered(false);
}
...
void MainWindow::on_actionCalibrate_2_triggered()
{
	...
	if ((ui->controller_iso->ch1_ref == 1.65) && (ui->controller_iso->ch2_ref == 1.65)
	    && (ui->controller_iso->frontendGain_CH1 == R4 / (R3 + R4))
	    && (ui->controller_iso->frontendGain_CH2 == R4 / (R3 + R4)))
	{
		calibrationMessages->setStandardButtons(QMessageBox::Ok);
		calibrationMessages->setText(
		    "You need to calibrate the oscilloscope before the power supply!");
		calibrationMessages->exec();
		return;
	}
	...
	calibrationMessages->setStandardButtons(QMessageBox::Ok | QMessageBox::Cancel);
	calibrationMessages->setText(
	    "Power Supply calibration requires me to control your power supply temporarily.  "
	    "\n\nTO PREVENT BLUE SMOKE DAMAGE, DISCONNECT ANY CIRCUIT ATTACHED TO YOUR POWER "
	    "SUPPLY NOW.");
	...
	calibrationMessages->setStandardButtons(QMessageBox::Ok);
	calibrationMessages->setText(
	    "Please connect your Labrador's Oscilloscope CH1 (DC) pin to the Power Supply "
	    "Output (positive) then press OK to continue.");
	calibrationMessages->exec();
...
void MainWindow::calibrate_psu_stage2()
{
	...
	if ((PSU5 > 6) | (PSU5 < 4))
	{
		...
		calibrationMessages->setText(
		    "Calibration has been abandoned due to out-of-range values.  The "
		    "oscilloscope should show approximately 5V.  Please check all wires on your "
		    "Labrador board and try again.");
		calibrationMessages->exec();
		return;
	}
...
void MainWindow::calibrate_psu_stage3()
{
	...
	if ((PSU10 > 12) | (PSU10 < 8))
	{
		calibrationMessages->setText(
		    "Calibration has been abandoned due to out-of-range values.  The "
		    "oscilloscope should show approximately 10V.  Please check all wires on your "
		    "Labrador board and try again.");
		calibrationMessages->exec();
		return;
	}
...
void MainWindow::on_actionShow_Range_Dialog_on_Main_Page_triggered(bool checked)
{
	...
	{
		scopeRangeSwitch = new scopeRangeEnterDialog(nullptr, false,
		    ui->controller_iso->display->topRange, ui->controller_iso->display->botRange,
		    ui->controller_iso->display->window, ui->controller_iso->display->delay);
		scopeRangeSwitch->setWindowFlags(Qt::Widget);
		ui->verticalLayout_5->insertWidget(2, scopeRangeSwitch);
		connect(scopeRangeSwitch, SIGNAL(yTopUpdated(double)), ui->controller_iso,
		    SLOT(setTopRange(double)));
		connect(scopeRangeSwitch, SIGNAL(yBotUpdated(double)), ui->controller_iso,
		    SLOT(setBotRange(double)));
		connect(scopeRangeSwitch, SIGNAL(windowUpdated(double)), ui->controller_iso,
		    SLOT(setTimeWindow(double)));
		connect(scopeRangeSwitch, SIGNAL(delayUpdated(double)), ui->controller_iso,
		    SLOT(setDelay(double)));

		connect(ui->controller_iso, SIGNAL(topRangeUpdated(double)), scopeRangeSwitch,
		    SLOT(yTopChanged(double)));
		connect(ui->controller_iso, SIGNAL(botRangeUpdated(double)), scopeRangeSwitch,
		    SLOT(yBotChanged(double)));
		connect(ui->controller_iso, SIGNAL(timeWindowUpdated(double)), scopeRangeSwitch,
		    SLOT(windowChanged(double)));
		connect(ui->controller_iso, SIGNAL(delayUpdated(double)), scopeRangeSwitch,
...
void MainWindow::on_actionFrequency_Spectrum_triggered(bool checked)
{
	...
	ui->controller_iso->display
	    = checked ? ui->controller_iso->display1 : ui->controller_iso->display0;
...
void MainWindow::on_actionFrequency_Response_triggered(bool checked)
{
	...
	ui->controller_iso->display
	    = checked ? ui->controller_iso->display2 : ui->controller_iso->display0;
...
void MainWindow::on_actionEye_Diagram_triggered(bool checked)
{
	...
	ui->scopeGroup_CH2->setDisabled(false);
	ui->controller_iso->display
	    = checked ? ui->controller_iso->display3 : ui->controller_iso->display0;
